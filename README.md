...
- **React + TypeScript** 控制台：每个浏览器会话都能看到动作状态（持有席位、
  剩余秒数、执行结果），并可申请 / 续期 / 释放 / 执行；**联排换班**时当前持有者
  可在租约仍有效时发起控制权转交——系统生成一次性交接码，接班席在同一动作卡片
  输入并确认后，旧租约终结、新令牌只返回给接班席；联排负责人还可在
...
1. pytest：针对同一套 PostgreSQL 验证**数据库事务竞争**（24 路并发争抢仅 1 个
   ...
   **执行历史**（事件编号倒序且字段齐全、联动两条事件共享标识且相邻、
   ...
   非法游标返回 `history_cursor_invalid`）以及**换班交接**（发起生成
   一次性交接码且只存哈希、原席接收前仍可续期 / 释放 / 执行、接收成功
   后旧令牌全部失效、错误码 / 已释放 / 已执行 / 到期 / 被取代 / 自我
   接收均为可识别拒绝且不生成新租约、待接收记录随查询判为失效、接收与
   原席执行并发仅一方提交、八路并发接收仅一人成功）；
2. Vitest：倒计时边界、接管 / 旧令牌界面逻辑，场次全流程组件测试
   （开始 → 两类动作执行 → 结束后摘要冻结），异常报告 / 确认组件测试
   （提交成功后轮询展示报告人与时间、空白说明就地反馈且不发请求、同一页面
   改席名不出现确认入口、新事件切换卡片后不误显旧异常），执行历史
   组件测试（最新一页倒序展示、联动归组、异常随事件展示、逐页加载更早
   记录、后续页失败时尾部重试且已加载内容保留），以及换班交接组件测试
   （发起后展示一次性交接码且原席保留全部权利、双控制台转交后旧令牌
   失效、同名两席仍按控制台身份判定已交接、发起席不见接收表单、过期
   交接拒绝且动作可重新申请、重新生成作废旧码）；
3. Playwright：两个真实浏览器上下文模拟双控制席——同时争抢只有一人持有、
   ...
   刷新后仍展示已确认结果与确认席位）、执行历史用例（执行单动作与
   联动、报告并确认异常后打开执行历史：最新一页倒序、联动相邻归组、
   场次与异常随事件展示、逐页加载更早记录无重复无跳页、整页刷新后
   重开历史仍得到一致记录，接口层核对游标翻页与非法游标错误码），以及
   换班交接用例（双浏览器成功转交后旧令牌失效、接收与原席执行并发
   仅一方成功、过期交接被 `handover_expired` 拒绝且动作仍可重新申请）。
...
联排换班时，当前持有者可把**仍有效的租约**直接转交给接班席，不必先释放
再争抢，也不必等 30 秒到期：

- 持有者在动作卡片上「发起换班交接」：`POST /api/actions/{id}/handover`
  以**当前租约令牌**鉴权（与续期 / 释放 / 执行同一套校验），生成一个
  **一次性交接码**（8 位无歧义字符，约 40 位熵；数据库只存 SHA-256
  哈希，原文仅在发起响应中出现一次）。发起**不触碰租约**——对方接收前，
  原席仍可续期、释放或执行；重新发起会在同一事务里作废旧记录并签发新码；
- 接班席（另一浏览器控制台）在同一动作卡片输入交接码并确认：
  `POST /api/actions/{id}/handover/accept` 在**一个事务**内锁定动作行
  （与执行同一把 `FOR UPDATE` 行锁），依次校验发起租约、交接状态与有效期，
  然后**终结旧租约**（标记 `released_at`）、**签发只返回给接班席的新令牌**
  （新租约 30 秒），并把交接记录置为已接收——全部成功或全部回滚，
  **任何失败都不生成新租约**；
- 交接记录存于 `lease_handovers`，复用动作、租约与控制台身份（发起 /
  接收双方稳定 console id 均落库），状态限定为**待接收 → 已接收 /
  已失效**（CHECK 约束保证三种形态的字段完整性；部分唯一索引保证同一
  动作至多一条待接收记录）。旧租约被释放、执行、到期或取代时，待接收
  记录**随查询判为失效**（与租约过期同构：失效是判断结果而非状态字段），
  不影响后续正常申请与再次发起；
- 接收方必须是**另一控制台**：发起控制台凭稳定 id 被识别，提交自己的
  交接码会被 `409 handover_self_accept` 拒绝（与异常留痕的"报告人不能
  自行确认"同理）；交接码错误 `409 handover_code_invalid`（记录保持
  待接收，可重试）、发起者已失去控制（租约已释放 / 执行 / 被取代）
  `409 handover_invalid`、接收时租约刚好到期 `409 handover_expired`
  均为可识别原因，错误响应附带最新动作 `state` 与当前交接记录，页面
  保留最新动作状态并提示重新发起；
- 接收与原席执行的并发由行锁串行化，**仅一方提交**：执行先提交则接收
  得到 `handover_invalid`，接收先提交则旧令牌执行得到 `control_lost`；
- 与既有契约完全兼容：申请、续期、释放、单动作与联动执行的路径与字段
  不变，动作快照仅**新增** `handover` 字段（不含交接码或其哈希）。两侧
  页面都靠轮询收敛：接班席接收成功后即获得持有控制件；交班席页面发现
  记录已接收（按控制台 id 判定，即使两席同名）后提示「控制权已交接」，
  旧令牌的任何操作仍被服务端以 `409 control_lost` 拒绝。
...
| POST | `/api/actions/{id}/handover` | 发起换班交接（body: `{"token","initiator_id"}`），返回一次性交接码；租约保持有效 |
| POST | `/api/actions/{id}/handover/accept` | 接班席接收交接（body: `{"code","recipient","recipient_id"}`），终结旧租约并仅向接班席返回新令牌 |
...
`400 invalid_handover`（接班席位或控制台身份空白）、
`409 handover_not_found`（当前没有待接收的交接）、
`409 handover_code_invalid`（交接码错误，记录保持待接收）、
`409 handover_invalid`（发起租约已释放 / 执行 / 被取代，交接失效）、
`409 handover_expired`（接收时租约刚好到期）、
`409 handover_self_accept`（发起控制台不能接收自己的交接码）；
交接类错误响应均附带最新动作 `state` 与当前交接记录，任何失败都不生成新租约。
