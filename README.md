...
- **React + TypeScript** 控制台：每个浏览器会话都能看到动作状态（持有席位、
  ...
  控制台**开始 / 结束场次**，把一段现场操作归入明确的一轮；演出复盘时打开
  **执行历史**，按发生顺序逐页追查每一次执行（含联动归组与异常留痕）；
...
1. pytest：针对同一套 PostgreSQL 验证**数据库事务竞争**（24 路并发争抢仅 1 个
   ...
   **场次归属**（单动作与联动事件在原事务内计入当前场次、并发开始仅一个
   ...
   且不改动事件、结束后摘要冻结且后续动作不再计入）、**现场异常留痕**
   （异常挂在最近一次执行事件上、重复报告 / 确认携带当前记录返回业务错误且
   首次数据不被改写、仅另一控制台可确认（同一页面改席名仍被稳定 id 拒绝）、
   16 路并发确认仅一人写入、新事件到来后旧记录保留但卡片
   不再显示、重新申请 / 释放 / 单动作 / 联动均不覆盖旧记录）与
   **执行历史**（事件编号倒序且字段齐全、联动两条事件共享标识且相邻、
   异常与确认结果随所属事件返回、游标翻页期间新执行不重复不跳过旧记录、
   非法游标返回 `history_cursor_invalid`）；
2. Vitest：倒计时边界、接管 / 旧令牌界面逻辑，场次全流程组件测试
   ...
   组件测试（最新一页倒序展示、联动归组、异常随事件展示、逐页加载更早
   记录、后续页失败时尾部重试且已加载内容保留）；
3. Playwright：两个真实浏览器上下文模拟双控制席——同时争抢只有一人持有、
   ...
   刷新后仍展示已确认结果与确认席位），以及执行历史用例（执行单动作与
   联动、报告并确认异常后打开执行历史：最新一页倒序、联动相邻归组、
   场次与异常随事件展示、逐页加载更早记录无重复无跳页、整页刷新后
   重开历史仍得到一致记录，接口层核对游标翻页与非法游标错误码）。
...
演出复盘时，负责人要按**发生顺序**追查每一次动作执行，而不是只能看到每张
卡片的最近结果。控制台的「执行历史」入口打开复盘视图：先展示**最新一页**，
列表尾部「加载更早记录」继续向过去翻页：

- 服务端提供只读接口 `GET /api/history`，以**事件编号倒序**返回执行时间、
  动作、席位与所属场次；同一次联动的两条事件共享 `link_id` 且在列表中
  **相邻归组**（共用一个联动标识分组头）；异常记录的内容、报告人、确认
  席位与确认时间**随所属事件**展示，不会串到相邻事件；
- 翻页使用**不可变事件编号作为游标**（keyset 分页）：响应携带
  `next_cursor`（本页最后一条事件编号），请求带回 `?cursor=` 即返回严格
  更早的一页（`WHERE id < cursor`）。翻页期间即使产生新执行，也只会落在
  已展示的第一页之上，**不会重复或跳过任何旧记录**；`next_cursor` 为
  `null` 表示没有更早的记录；
- 页边界**绝不拆开同一次联动**：联动两条事件编号相邻，当页边界恰好落在
  两者之间时，伴侣事件并入本页（该页因此多一条），任一屏的联动分组都
  完整相邻，不会出现只含一个动作的残缺分组；
- 历史查询在**一条 SELECT** 内联动作、场次与异常记录（`action_events ⋈
  actions ⋉ rehearsal_sessions ⋉ action_anomalies`），不逐条补查关联信息；
  接口只读，不触碰租约、事件、场次或异常数据；
- 游标非法（非正整数事件编号）返回可识别的 `400 history_cursor_invalid`；
  页面保留已加载内容。后续页请求失败（含游标非法）时，已加载记录保持
  不变，**列表尾部给出错误与重试入口**，重试成功后在原位置继续追加；
- 页面大小由前端固定（10 条/页），服务端将上限钳制在 50 条以内。
...
| GET | `/api/history` | 只读执行历史（query: `cursor` 上一页的 next_cursor、`limit` 页大小），事件编号倒序，含场次、联动标识与随事件的异常记录 |
...
`400 history_cursor_invalid`（历史游标不是有效事件编号，已加载内容保留）。
...
backend/            FastAPI + psycopg + PostgreSQL
  ...
  app/history.py      只读执行历史（事件编号游标分页，单查询关联场次/联动/异常）
...
frontend/           React + Vite + TypeScript
  ...
  src/App.tsx         短轮询控制台（含场次控制、本轮摘要、卡片异常留痕与执行历史入口）
  src/HistoryPanel.tsx 执行历史复盘视图（最新一页 → 逐页加载更早，尾部重试）
